...
bool isValidIdCardNumber(String value) {
  return isChineseIdCardNumber(value, allowLowercaseX: true);
}
...
class _VehicleListState extends State<VehicleList> {
  ...
  Future<void> _checkUserRole() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(
            () => _errorMessage = 'vehicle.error.roleCheckFailed'.trParams({
                  'error': 'vehicle.error.httpStatus'
  ...
  Future<void> _fetchVehicles({bool reset = false, String? query}) async {
    ...
    try {
      ...
      List<VehicleInformation> vehicles = await vehicleApi.apiVehiclesGet();
      ...
      setState(() {
        if (e is ApiException && e.code == 403) {
          ...
          _errorMessage =
              'vehicle.error.loadFailed'.trParams({'error': formatVehicleError(e)});
        }
      });
    } finally {
      setState(() => _isLoading = false);
...
class _AddVehiclePageState extends State<AddVehiclePage> {
  ...
  Future<void> _submitVehicle() async {
    ...
    if (!isValidIdCardNumber(idCardNumber)) {
      _showSnackBar('vehicle.validation.idCardInvalid'.tr, isError: true);
      return;
    }
    setState(() => _isLoading = true);
    try {
      final vehiclePayload = {
        ...
        'firstRegistrationDate':
            _selectedFirstRegistrationDate?.toIso8601String(),
      ...
      if (response.statusCode != 201) {
        throw Exception('vehicle.error.httpStatus'
            .trParams({'statusCode': '${response.statusCode}'}));
      }
...
class _VehicleDetailPageState extends State<VehicleDetailPage> {
  ...
  Future<void> _checkUserRole() async {
    try {
      ...
      final jwtToken = (await AuthTokenStore.instance.getJwtToken());
      if (jwtToken == null) {
        setState(() => _errorMessage = 'vehicle.error.jwtMissingRelogin'.tr);
        return;
      }
      final response = await http.get(
        Uri.parse('http://localhost:8081/api/users/me'),
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            'Role check failed: Status ${response.statusCode}, Body: ${response.body}');
        setState(() =>
            _errorMessage = 'vehicle.error.permissionLoadFailed'.trParams({
              'error': 'vehicle.error.httpStatus'
                  .trParams({'statusCode': '${response.statusCode}'})
            }));
        return;
